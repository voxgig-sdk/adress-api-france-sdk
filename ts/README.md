# AdressApiFrance TypeScript SDK



The TypeScript SDK for the AdressApiFrance API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.BatchGeocoding()` — each with a small set of operations (`list`, `create`)
...
| `tester(testopts?, sdkopts?)` | `AdressApiFranceSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `AdressApiFranceSDK.test(testopts?, sdkopts?)` | `AdressApiFranceSDK` | Create a test-mode client. |
...
| `client` | `client(): AdressApiFranceSDK` | Return the parent SDK client. |
...
```ts
const geocodings = await client.Geocoding().list({ q: "example" })
```
...
adress-api-france/
├── src/
│   ├── AdressApiFranceSDK.ts        # Main SDK class
