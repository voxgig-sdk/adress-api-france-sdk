...
describe('GeocodingDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new AdressApiFranceSDK({
  ...
  test('direct-list-geocoding', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-geocoding', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ADRESS_API_FRANCE_TEST_GEOCODING_ENTID']
