...
//     (\`new AdressApiFranceSDK(...)\` / \`AdressApiFranceSDK.test(...)\` become
//     \`AdressApiFranceSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'AdressApiFranceSDK'
...
const TEST_SEED = {"entity":{"batch_geocoding":{"test01":{"id":"test01"}},"geocoding":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AdressApiFranceSDK(...)\` and any
// \`AdressApiFranceSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AdressApiFranceSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AdressApiFranceSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
