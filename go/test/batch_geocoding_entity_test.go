...
func TestBatchGeocodingEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ADRESS_API_FRANCE_TEST_BATCH_GEOCODING_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		batchGeocodingRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "batch_geocoding"}), "batch_geocoding_ref01"))

		batchGeocodingRef01DataResult, err := batchGeocodingRef01Ent.Create(batchGeocodingRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		batchGeocodingRef01Data = core.ToMapAny(entityData(batchGeocodingRef01DataResult))
...
func batch_geocodingBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ADRESS_API_FRANCE_TEST_BATCH_GEOCODING_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ADRESS_API_FRANCE_TEST_BATCH_GEOCODING_ENTID"])
