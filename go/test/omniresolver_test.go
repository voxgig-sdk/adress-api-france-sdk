...
import (
	...
	omni "github.com/voxgig-sdk/adress-api-france-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AdressApiFranceError); is {
				out := map[string]any{
					"name":    "AdressApiFranceError",
...
func omniCtx(arg any, client *sdk.AdressApiFranceSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AdressApiFranceSDK); is {
			client = live
