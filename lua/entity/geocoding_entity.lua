...
local GeocodingEntity = {}
GeocodingEntity.__index = GeocodingEntity


function GeocodingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GeocodingEntity)
  self._name = "geocoding"
  self._client = client
...
function GeocodingEntity:get_name()
  return self._name
end


function GeocodingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GeocodingEntity.new(self._client, opts)
end
...
function GeocodingEntity:mark_deleted()
  self._deleted = true
end


function GeocodingEntity:deleted()
  return true == self._deleted
end


function GeocodingEntity:data_set(args)
  if args ~= nil then
...
function GeocodingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GeocodingEntity:match_set(args)
  if args ~= nil then
...
function GeocodingEntity:match_get()
...
function GeocodingEntity:stream(action, args, callopts)
...
function GeocodingEntity:_run_op(ctx, post_done)
  local utility = self._utility
