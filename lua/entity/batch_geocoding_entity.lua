...
local BatchGeocodingEntity = {}
BatchGeocodingEntity.__index = BatchGeocodingEntity


function BatchGeocodingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BatchGeocodingEntity)
  self._name = "batch_geocoding"
  self._client = client
...
function BatchGeocodingEntity:get_name()
  return self._name
end


function BatchGeocodingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BatchGeocodingEntity.new(self._client, opts)
end
...
function BatchGeocodingEntity:mark_deleted()
  self._deleted = true
end


function BatchGeocodingEntity:deleted()
  return true == self._deleted
end


function BatchGeocodingEntity:data_set(args)
  if args ~= nil then
...
function BatchGeocodingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BatchGeocodingEntity:match_set(args)
  if args ~= nil then
...
function BatchGeocodingEntity:match_get()
...
function BatchGeocodingEntity:stream(action, args, callopts)
...
---@param reqdata BatchGeocodingCreateData
---@param ctrl? table
---@return BatchGeocoding
---@return string? err
function BatchGeocodingEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function BatchGeocodingEntity:_run_op(ctx, post_done)
  local utility = self._utility
