...
require("adress-api-france_types")
...
local AdressApiFranceSDK = {}
AdressApiFranceSDK.__index = AdressApiFranceSDK
...
AdressApiFranceSDK._make_feature = _make_feature


function AdressApiFranceSDK.new(options)
  local self = setmetatable({}, AdressApiFranceSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function AdressApiFranceSDK:options_map()
...
function AdressApiFranceSDK:get_utility()
  return Utility.copy(self._utility)
end


function AdressApiFranceSDK:get_root_ctx()
  return self._rootctx
end


function AdressApiFranceSDK:prepare(fetchargs)
...
function AdressApiFranceSDK:direct(fetchargs)
...
function AdressApiFranceSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function AdressApiFranceSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "AdressApiFranceSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function AdressApiFranceSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function AdressApiFranceSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "AdressApiFranceSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function AdressApiFranceSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = AdressApiFranceSDK.new(sdkopts)
  sdk.mode = "test"
