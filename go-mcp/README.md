# adress-api-france-mcp

[MCP](https://modelcontextprotocol.io) server exposing the AdressApiFrance SDK as
two agent tools — `adress-api-france_list` and `adress-api-france_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/adress-api-france-mcp)
make build

# 2. Provide credentials via the environment
export ADRESS_API_FRANCE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user adress-api-france \
  -- /absolute/path/to/adress-api-france-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./adress-api-france-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// adress-api-france_list: first page of records
{ "entity": "geocoding" }
{ "entity": "geocoding", "query": { } }

// adress-api-france_load: one record by id
{ "entity": "batch_geocoding", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/adress-api-france-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user adress-api-france \
     -- "$PWD"/dist/*/adress-api-france-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `adress-api-france_list` and `adress-api-france_load` tools now appear
   in new sessions. Ask the agent to *"list geocoding using adress-api-france"*
   and it calls `adress-api-france_list` with `{"entity":"geocoding"}`.
...
```sh
export ADRESS_API_FRANCE_APIKEY=sk_live_xxx            # API key
export ADRESS_API_FRANCE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `adress-api-france_list` tool
...
### Call the `adress-api-france_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `adress-api-france_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `adress-api-france_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ADRESS_API_FRANCE_APIKEY` | API key sent with every request. |
| `ADRESS_API_FRANCE_BASE` | Optional override of the API base URL. |

### Entities

The 2 entities valid as the `entity` argument:

batch_geocoding | geocoding

### Smoke test via HTTP (raw JSON-RPC)

```sh
./adress-api-france-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"adress-api-france_load","arguments":{"entity":"batch_geocoding","query":{"id":1}}}}'
```
