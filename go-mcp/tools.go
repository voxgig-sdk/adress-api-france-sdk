...
type Args struct {
	Entity string         `json:"entity" jsonschema:"batch_geocoding | geocoding"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.AdressApiFranceSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "adress-api-france_list",
		Description: "List records from AdressApiFrance. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "adress-api-france_load",
		Description: "Load a single record from AdressApiFrance. " +
...
func runOp(client *sdk.AdressApiFranceSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.AdressApiFranceSDK, name string) (sdk.AdressApiFranceEntity, error) {
	switch strings.ToLower(name) {
	case "batch_geocoding":
		return client.BatchGeocoding(nil), nil
	case "geocoding":
		return client.Geocoding(nil), nil
