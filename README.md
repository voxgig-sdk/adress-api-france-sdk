...
Learn more about French Address API at [adresse.data.gouv.fr/api-doc/adresse](https://adresse.data.gouv.fr/api-doc/adresse).
...
// geocodings is an array of Geocoding entities, populated with mock data
// — call geocodings[0].data() for the record itself
...
```go
client := sdk.Test()
result, err := client.Geocoding(nil).List(
    nil, nil,
)
```
