# AdressApiFrance Ruby SDK



The Ruby SDK for the AdressApiFrance API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.BatchGeocoding` — with named operations (`list`/`create`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = AdressApiFranceSDK.test(testopts, sdkopts)
```
...
raise a `AdressApiFranceError` (a `StandardError` subclass), so wrap
...
```
rb/
├── AdressApiFrance_sdk.rb       -- Main SDK module
...
The main module (`AdressApiFrance_sdk`) exports the SDK class
