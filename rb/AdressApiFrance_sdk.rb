...
require_relative 'AdressApiFrance_types'
...
class AdressApiFranceSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = AdressApiFranceHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = AdressApiFranceHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, AdressApiFranceFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    AdressApiFranceUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = AdressApiFranceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = AdressApiFranceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = AdressApiFranceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue AdressApiFranceError => err
    ...
    if fetched.is_a?(Hash)
      status = AdressApiFranceHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = AdressApiFranceError.new(
        "graphql_error", "AdressApiFranceSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = AdressApiFranceSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
