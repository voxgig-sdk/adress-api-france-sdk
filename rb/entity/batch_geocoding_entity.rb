...
class BatchGeocodingEntity
  ...
  def make
    opts = @_entopts.dup
    BatchGeocodingEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AdressApiFranceHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [BatchGeocoding, Hash] the current BatchGeocoding data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = AdressApiFranceHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of BatchGeocoding fields)
  def match_get
  ...
  #
  # @param reqdata [BatchGeocodingCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [BatchGeocoding, Hash] the created BatchGeocoding; raises AdressApiFranceError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = AdressApiFranceHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
