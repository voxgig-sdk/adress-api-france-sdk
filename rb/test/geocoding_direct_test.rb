...
class GeocodingDirectTest < Minitest::Test
  def test_direct_list_geocoding
    setup = geocoding_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-geocoding", setup[:live] ? "live" : "unit")
    if _should_skip
...
def geocoding_direct_setup(mockres)
