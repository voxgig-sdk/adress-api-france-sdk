...
// AdressApiFrance SDK feature test
//
...
class FtCtrl extends AdressApiFranceControl
{
...
class FtClient
{
    ...
    private AdressApiFranceUtility $utility;

    public function __construct(AdressApiFranceUtility $utility, array $options)
    {
    ...
    public function get_utility(): AdressApiFranceUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AdressApiFranceContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AdressApiFranceContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AdressApiFranceContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AdressApiFranceConfig::make_feature($name);
    ...
    private static function buildUrl(AdressApiFranceSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AdressApiFranceError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AdressApiFranceResponse($fetched);
        }
    ...
    private function fail(AdressApiFranceContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AdressApiFranceContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
