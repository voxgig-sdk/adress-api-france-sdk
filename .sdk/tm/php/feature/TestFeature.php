...
// AdressApiFrance SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class AdressApiFranceTestFeature extends AdressApiFranceBaseFeature
{
    ...
    public function init(AdressApiFranceContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (AdressApiFranceContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = AdressApiFranceParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (AdressApiFranceContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(AdressApiFranceContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = AdressApiFranceParam::call($ctx, $k);
