...
// AdressApiFrance SDK retry feature
...
class AdressApiFranceRetryFeature extends AdressApiFranceBaseFeature
{
    ...
    public function init(AdressApiFranceContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AdressApiFranceContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AdressApiFranceContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AdressApiFranceContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
