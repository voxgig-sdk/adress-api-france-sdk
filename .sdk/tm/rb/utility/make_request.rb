...
module AdressApiFranceUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AdressApiFranceResponse.new({})
    ...
    if fetch_err
      ...
      response = AdressApiFranceResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AdressApiFranceResponse.new(fetched)
    else
