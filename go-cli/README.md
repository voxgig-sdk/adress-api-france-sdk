...
boru-driven command-line client **and** interactive REPL for the AdressApiFrance
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/adress-api-france-cli)
make build

# 2. See usage (words, entities, env vars)
./adress-api-france-cli --help
...
ADRESS_API_FRANCE_BASE=https://api.example.com ./adress-api-france-cli --help

# 6. No arguments -> interactive REPL
./adress-api-france-cli
adress-api-france> /help
adress-api-france> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/adress-api-france-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/adress-api-france-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `adress-api-france>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `batch_geocoding`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/adress-api-france-cli`. |
...
(`./adress-api-france-cli <expr>`) and interactively (the REPL), and expressions compose the
