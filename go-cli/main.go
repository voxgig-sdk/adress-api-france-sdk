// Command AdressApiFrance-cli is a CLI + REPL for the AdressApiFrance SDK,
...
const prompt = "adress-api-france"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "batch_geocoding geocoding"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      ADRESS_API_FRANCE_APIKEY (api key), ADRESS_API_FRANCE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: ADRESS_API_FRANCE_APIKEY carries the API key and
	// ADRESS_API_FRANCE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ADRESS_API_FRANCE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ADRESS_API_FRANCE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAdressApiFranceSDK(opts)
