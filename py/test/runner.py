# AdressApiFrance SDK test runner
...
class AdressApiFranceTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AdressApiFranceTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AdressApiFranceTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AdressApiFranceTestRunner.getenv("ADRESS_API_FRANCE_TEST_LIVE")
        override = AdressApiFranceTestRunner.getenv("ADRESS_API_FRANCE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AdressApiFranceTestRunner.getenv(key)
        ...
        explain = AdressApiFranceTestRunner.getenv("ADRESS_API_FRANCE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["ADRESS_API_FRANCE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if AdressApiFranceTestRunner._test_control is not None:
            return AdressApiFranceTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AdressApiFranceTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AdressApiFranceTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AdressApiFranceTestRunner.load_test_control()
...
def load_env_local():
    AdressApiFranceTestRunner.load_env_local()


def env_override(m):
    return AdressApiFranceTestRunner.env_override(m)


def entity_data(v):
    return AdressApiFranceTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AdressApiFranceTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AdressApiFranceTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AdressApiFranceTestRunner.load_test_control()


def live_client_options():
    return AdressApiFranceTestRunner.live_client_options()


def live_delay_ms():
    return AdressApiFranceTestRunner.live_delay_ms()
