...
// AdressApiFrance SDK BatchGeocoding entity
...
class BatchGeocodingEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new BatchGeocodingEntity($this->_client, $opts);
    }

    /**
     * @param BatchGeocoding|array $args BatchGeocoding data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = AdressApiFranceHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return BatchGeocoding|array The current BatchGeocoding data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of BatchGeocoding fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = AdressApiFranceHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of BatchGeocoding fields).
     */
    ...
    /**
     * Create a new BatchGeocoding.
     *
     * @param BatchGeocodingCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed BatchGeocodingCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return BatchGeocoding|array The created BatchGeocoding as an assoc-array at the
     *   SDK boundary; throws AdressApiFranceError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = AdressApiFranceHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
