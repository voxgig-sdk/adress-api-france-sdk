...
class GeocodingDirectTest extends TestCase
{
    public function test_direct_list_geocoding(): void
    {
        $setup = geocoding_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-geocoding", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function geocoding_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new AdressApiFranceSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
